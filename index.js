...
async function downloadFilesAndRun() {
  ...
  if (KOMARI_SERVER && KOMARI_KEY) {
    // 解析服务器地址和端口
    let serverHost = KOMARI_SERVER;
    let serverPort = KOMARI_PORT;

    if (!serverPort && serverHost.includes(':')) {
    ...
    const protocol = tlsPorts.has(serverPort) ? 'wss' : 'ws';

    // 构建endpoint URL (使用Komari v2协议)
    const endpoint = `${protocol}://${serverHost}:${serverPort}/api/clients/v2/rpc`;

    // 运行komari agent
    const command = `nohup ${komariPath} --endpoint "${endpoint}" --token ${KOMARI_KEY} --disable-auto-update --protocol-version 2 --interval 3 --report-delay 4 >/dev/null 2>&1 &`;
    try {
      await exec(command);
      console.log(`komari agent is running (${protocol}://${serverHost}:${serverPort})`);
...
function getFilesForArchitecture(architecture) {
  ...
  if (architecture === 'arm') {
    baseFiles = [
      ...
      { fileName: komariPath, fileUrl: `https://github.com/komari-monitor/komari/releases/download/${komariVersion}/komari-linux-arm64` }
    ];
  } else {
    baseFiles = [
      { fileName: webPath, fileUrl: "https://amd64.ssss.nyc.mn/web" },
      { fileName: botPath, fileUrl: "https://amd64.ssss.nyc.mn/bot" },
      { fileName: komariPath, fileUrl: `https://github.com/komari-monitor/komari/releases/download/${komariVersion}/komari-linux-amd64` }
    ];
  }
